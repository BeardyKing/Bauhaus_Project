...
public class EnemyTwoManager : MonoBehaviour {
	...
	void SetNewPosition() {
		...
		if (counter >= timeBettweenChange) {
			if (StaticData.GameObjectList[x + dir[0], y + dir[1]].GetComponent<ChangeColour>().startCol == 1) {
				OnPath();
			} else if (StaticData.GameObjectList[x + dir[0], y + dir[1]].GetComponent<ChangeColour>().startCol == 3) {
				CollidedWithSelf();
			} else {
				CollidedWithWall();
			}
	...
	void OnPath() {
		StaticData.GameObjectList[x + dir[0], y + dir[1]].GetComponent<ChangeColour>().startCol = 3;
		hasMoved = true;
		RemoveLastPosition();
		x = x + dir[0];
		y = y + dir[1];
	...
	bool CheckIfSquare() {
		try {
			...
			for (int i = 0; i < 4; i++) {
				...
				//StaticData.GameObjectList[(int)pos[i].x, (int)pos[i].y].GetComponent<ChangeColour>().startCol = 6; //TODO CUNCOMMENT IF EVERYTHING STOPS WORKING
				...
				if ((int)pos[0].x == (int)pos[3].x || (int)pos[0].y == (int)pos[3].y) {
					if ((int)pos[2].x == (int)pos[1].x || (int)pos[2].y == (int)pos[1].y) {
						if ((int)pos[2].x == (int)pos[3].x || (int)pos[2].y == (int)pos[3].y) {
							...
							for (int i = (int)topLeft.x; i < (int)bottomRight.x + 1; i++) {
								for (int j = (int)topLeft.y; j < (int)bottomRight.y + 1; j++) {
									StaticData.GameObjectList[i, j].GetComponent<ChangeColour>().startCol = 6;
								}
							}
